...
class fun(commands.Cog):
    ...
    #list can be extended
        esponses = [ 'Eh.. It seems that you have met a-a horrible demise my friend. But.. uh.. you know these.. These things happen in - in life. Life goes on. Not for you obviously uh youre dead but uh.. It reminds me of a time. I was - I was having a conversation with my friend Orville. We were uh.. we were.. - the river. We were sitting by the river watching the fish leap over the falls and I said to Orville: Sometimes I feel like a fish leaping over and over again always trying to get somewhere. No I dont know where...or only to find to find myself in the jaws of a beast. He of course looked at me.. eh.. surprised you know? Have you been in the jaws of a beast friend? To which I said No of course not Orville. I said No no no no no. I simply meant that life can seem like a relentless endeavor... Overcome meaningless obstacles only to meet an equally meaningless fate regardless of your efforts. Regardless of the obstacles youve passed. And.. uh.. Orville he stood and proceeded to drape me with a picnic cloth to which I asked him I said Friend what - what are you doing? He looked at me... very concerned really. I feel like youve gotten too much sun. Indeed heh. Indeed I had. He proceeded to pour me a glass of... just… ice-cold lemonade... Ooh. Ever mix it with iced tea? I do like... little half-lemonade half-...oh its so - you should try it someti- oh wait. You cant because youre dead. But anyways...So you may be asking yourself How did I go from sitting by the falls to drinking lemonade to being wedged in the air duct? Not only with Orville but with an entire assortment of fruity colored friends. Well theres.. uh.. Theres really no good answer to that but perhaps Ive met a demise of my own at some point and this is my afterlife or my dream - whatever it might be. I honestly dont know... Or... Maybe it doesnt mean anything at all. Maybe it doesnt mean anything at all.',
        ...
        await ctx.channel.purge(limit=1) #this will for the most part delete the command that called the copypasta. It might fail sometimes if the person is quick fingers mcgee tho but it would be hard for a human to mess it up
    ...
    async def hug(self, ctx, user : discord.Member):
        if user != ctx.message.author:
            await ctx.channel.send(f"{ctx.message.author.mention} gives you hugs {user.mention}（っ・∀・）っ ⊂(・ヮ・⊂)", delete_after=10800)
        if user == ctx.message.author:
            await ctx.channel.send(f"{ctx.message.author.mention} hugs themself. What a lonely chap", delete_after=10800)
           #kisses a user
    @commands.command(aliases=["muah"])
    @commands.cooldown(1, 2, commands.BucketType.user)
    async def kiss(self, ctx, user : discord.Member):
        if user != ctx.message.author:
            await ctx.channel.send(f"{ctx.message.author.mention} kisses {user.mention}! Idk, seems kinda horny to me.", delete_after=10800)
        if user == ctx.message.author:
            await ctx.channel.send(f"{ctx.message.author.mention} kisses themself. What a lonely and mega horny chap. Everybody laugh at this idot", delete_after=10800)
    #specifically tailored to my server. feel free to disable.
    @commands.command(aliases=["fuck", "seks"])
    @commands.cooldown(1, 500, commands.BucketType.user)
    async def uwu(self, ctx, user : discord.User):
        role = discord.utils.get(ctx.guild.roles, name="the most horny person in existence")
        await ctx.channel.send(f"You are so fucking horny {ctx.message.author.mention} holy shit. Gtfo i swear this is not ok", delete_after=30)
